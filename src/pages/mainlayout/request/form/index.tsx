...
import { RejectedApprovalForm, RejectedApprovalFooter } from './components/rejected_approval'
...
const RequestForm = () => {
    ...
    const handleData = async () => {
        if (!params.id) navigate(-1)
            ...
            var res:any = await global.$baseAxios.get(`/internaluser/v1/approval/${params.id}`)
            setData(res?.data)
        }
    }
    ...
    const handleApproval = async (status:string) => {
        if (status === 'approved') dispatch({ type: DIALOG_ACTIVE, dialogCustomization: { dialogImage: 'success', title: 'Konfirmasi', description: 'Apakah anda yakin menyetujui approval ini' } })
        else dispatch({ type: DIALOG_ACTIVE, dialogCustomization: { childrenVar: [<RejectedApprovalForm slot='dialog-body' />, <RejectedApprovalFooter slot="dialog-footer" />], dialogImage: 'error' } })
        const confirm = await execute()
        var payload = {}
        if (typeof confirm === 'object') payload = {...confirm, status: status}
        else payload = {status: status}
        await global.$baseAxios.put(`/internaluser/v1/approval/${params.id}/status`, payload)
        navigate(-1)
    }
            ...
            <StickyComponent>
                <div className="w-full flex justify-center py-4 border border-solid border-t-0 border-r-0 border-l-0">
                    <div className="container flex justify-between sm:flex-col md:flex-row flex-col gap-5">
                        ...
                        <div className="flex gap-5">
                            <button className="button-success-dark font-roboto my-auto p-2 w-[120px]" onClick={() => handleApproval('approved')}>Approve</button>
                            <button type="button" className="button-border-error font-roboto my-auto p-2 w-[120px]" onClick={() => handleApproval('rejected')}>Reject</button>
