...
import StatusComponent from "components/general/Status"
...
const ListUser = () => {
    ...
    const handleEdit = (params:any) => {
        navigate(`/form/user/${params.data.id}`)
    }
    ...
    const actions = [
        ...
        {
            name: 'Activate',
            show: permissions.findIndex((a:any) => a.name === 'User Update Status') !== -1,
            action: (data:any) => handleUpdate(data, 'active')
        },
        {
            name: 'Deactivate',
            show: permissions.findIndex((a:any) => a.name === 'User Update Status') !== -1,
            action: (data:any) => handleUpdate(data, 'inactive')
        }
    ]
    ...
    const columnDefs:(ColDef)[] = [
        ...
        {colId: 'email', headerName: "Email", field: "email", filter: true, sortable: true},
        {colId: 'name', headerName: "Name", field: "name",},
        {colId: 'role_name', headerName: "Role", field: 'role_name', sortable: true, filter: true},
        {colId: 'status', headerName: "Status", field: 'status', headerClass: 'ag-header-text-center', cellRenderer: (params:any) => {
    ...
    const handleUpdate = async (data:any, status:string) => {
        ...
        dispatch({ type: DIALOG_ACTIVE, dialogCustomization: { title: 'Konfirmasi', dialogImage: (status === 'active') ? 'success' : 'error', description: 'Apakah anda yakin untuk mengubah ' +  data.data.name + ' status menjadi ' + status } })
        ...
        var payload = {
            ...
            type: 'update_user',
            action: "update",
            reason: 'merubah status ' + data.data.name + ' menjadi ' + status,
            details: {
                old_data: data.data,
                new_data: {...data.data, status: status}
            }
        }
    ...
    const initData = async () => {
        ...
        var res:any = await global.$baseAxios.get('/internaluser/v1/user', {params: payload})
        setRowData(res?.data)
